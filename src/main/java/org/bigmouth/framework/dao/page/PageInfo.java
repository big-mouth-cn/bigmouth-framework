...
public class PageInfo<T> extends RowBounds {
    ...
    public void initPageSource() {
        ...
        pathfind.append("<ul skymobiEscapeHtml=\"false\" class='pagination'>");
        ...
        // --
        if (isHasPre()) {
            pathfind.append("<li><a href=\"javascript:Page.jump(" + this.getPrePage() + ")\">&laquo;</a></li>");
        }
        else {
        ...
        if (m == 2) {
            pathfind.append("<li><a href=\"javascript:Page.jump(1)\"> 1 </a></li>");
        }
        if (m > 2) {
            pathfind.append("<li><a href=\"javascript:Page.jump(1)\"> 1 </a>" + "<span>...</span></li>");
        }
        for (int i = m; i <= n; i++) {
            if (i == n) {
                if (i == this.getPageNo()) {
                    ...
                    pathfind.append("<li><a href=\"javascript:Page.jump(" + i + ")\">" + i + "</a></li>");
                }
            }
            else {
                if (i == this.getPageNo()) {
        ...
        if (this.getPageNo() != this.getTotalPage() && this.getTotalPage() > baseNum
            ...
            if (this.getTotalPage() == (this.getPageNo() + betweenNum + 1)) {
                pathfind.append("<li><a href=\"javascript:Page.jump(" + this.getTotalPage() + ")\"> "
                        + this.getTotalPage() + " </a></li>");
            }
            else {
                pathfind.append("<li><span>...</span><a href=\"javascript:Page.jump(" + this.getTotalPage() + ")\"> "
                        + this.getTotalPage() + " </a></li>");
            }
        }
        if (isHasNext()) {
            pathfind.append("<li><a href=\"javascript:Page.jump(" + this.getNextPage() + ")\">&raquo;</a></li>");
        }
        else {
